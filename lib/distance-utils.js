...
async function getLatLngPelias(address) {
    const url = `http://pelias.mapc.org/v1/search?text=${address}&boundary.gid=whosonfirst:region:85688645&size=1`;
    const response = await axios.get(url);
    return getPeliasLatLong(response.data);
}
 ...
 */
function getPeliasLatLong(geojson) {
 ...
 * caller should handle catching exceptions (likely index/key errors).
 */
 function getPeliasAddress(geojson) {
    if (geojson['features'].length == 0) {
        throw 'Pelias Geocoding: No results';
    }
...
async function getPeliasStreetAddress(lat, long) {
    const url = `http://pelias.mapc.org/v1/reverse?point.lat=${lat}&point.lon=${long}&boundary.gid=whosonfirst:region:85688645&size=1`;
    const response = await axios.get(url);
    return getPeliasAddress(response.data);
}
...
module.exports = {
    ...
    getPeliasStreetAddress: getPeliasStreetAddress
};
